...
class blocks_attacks:
    block_delay_seconds: Annotated[float, 'Range | `0` and above | inclusive'] | None = None  # Number of seconds that right-click must be held before successfully blocking attacks. Defaults to `0`.
    disable_cooldown_scale: Annotated[float, 'Range | `0` and above | inclusive'] | None = None  # Multiplier applied to the number of seconds that the item will be on cooldown for when attacked by a disabling attack (`disable_blocking_for_seconds` on the `weapon` component). Defaults to `1`. If `0`, this item can never be disabled by attacks.
